 ...
 * different from the 'Gaussian Blur' in ImageJ versions before 1.38u, where a
 * value 2.5 times as much had to be entered. - Like all convolution operations
 * in ImageJ, it assumes that out-of-image pixels have a value equal to the
 * nearest edge pixel. This gives higher weight to edge pixels than pixels
 * inside the image, and higher weight to corner pixels than non-corner pixels
 * at the edge. Thus, when smoothing with very high blur radius, the output will
 * be dominated by the edge pixels and especially the corner pixels (in the
 * extreme case, with a blur radius of e.g. 1e20, the image will be raplaced by
 * the average of the four corner pixels). - For increased speed, except for
 * small blur radii, the lines (rows or columns of the image) are downscaled
 * before convolution and upscaled to their original length thereafter.
 *
 * Version 03-Jun-2007 M. Schmid with preview, progressBar stack-aware, snapshot
 * via snapshot flag; restricted range for resetOutOfRoi
 *
...
public class GaussianBlur {
     ...
     */
    public static void blur(ImageProcessor ip, double sigma) {
        FloatProcessor fp = ip.convertToFloatProcessor();

        double accuracy = 0.002;
        blurFloat(fp, sigma, sigma, accuracy);
        ip.setPixels(0, fp);
    }

    /**
     * Gaussian Filtering of a FloatProcessor. This method does NOT include
     * resetOutOfRoi(ip), i.e., pixels above and below the roi rectangle will be
     * also subject to filtering in x direction and must be restored afterwards
     * (unless the full image height is processed).
     *
     ...
     * @param accuracy Accuracy of kernel, should not be above 0.02. Better
     * (lower) accuracy needs slightly more computing time.
     */
     ...
     *
     * @param ip The Image with the original data where also the result will be
     * stored
     ...
     */
    public static void blur1Direction(final FloatProcessor ip, final double sigma, final double accuracy,
        ...
        final int lineTo = (xDirection ? height : width);
        ...
        final int newLength = doDownscaling
    ...
    /**
     * Scale a line (row or column of a FloatProcessor or part thereof) down by
     * a factor <code>reduceBy</code> and write the result into
     * <code>cache</code>. Input line pixel # <code>unscaled0</code> will
     * correspond to output line pixel # 0. <code>unscaled0</code> may be
     * negative. Out-of-line pixels of the input are replaced by the edge
     * pixels.
     *
     ...
     * column
     * @param unscaled0 index in input line corresponding to output line index
     * 0, May be negative.
     * @param length length of full input line or column
     * @param pointInc spacing of values in input array (1 for lines, image
     * width for columns)
     ...
     * faster - above: accesses each pixel in the pixels array only once -
 ...
 /* Create a kernel for downscaling. The kernel function preserves
    ...
    /**
     * Scale a line up by factor <code>reduceBy</code> and write as a row or
     * column (or part thereof) to the pixels array of a FloatProcessor.
     */
    ...
    /**
     * Create a kernel for upscaling. The kernel function is a convolution of
     * four unit squares, i.e., four uniform kernels with value +1 from -0.5 to
     * +0.5 (in downscaled coordinates). The second derivative of this kernel is
     * smooth, the third is not. Its standard deviation is 1/sqrt(3) in
     * downscaled cordinates. The kernel runs from [-2 to +2[, corresponding to
     ...
     * @param writeFrom Index of the first point in the line that should be
     * written
     * @param writeTo Index+1 of the last point in the line that should be
     * written
     * @param point0 Array index of first element of the 'line' in pixels (i.e.,
     * lineNumber * lineInc)
     * @param pointInc Increment of the pixels array index to the next point
     ...
     * sigma and the running sum over the kernel Note: this is one side of the
     * kernel only, not the full kernel as used by the Convolver class of
     * ImageJ. To avoid a step due to the cutoff at a finite value, the
     * near-edge values are replaced by a 2nd-order polynomial with its
     * minimum=0 at the first out-of-kernel pixel. Thus, the kernel function has
     * a smooth 1st derivative in spite of finite length.
     *
     ...
     * processing 8-bit images. For short or float images, values of 1e-3 to
     * 1e-4 are better (but increase the kernel size and thereby the processing
     * time). Edge smoothing will fail with very poor accuracy (above approx.
     * 0.02)
     * @param maxRadius Maximum radius of the kernel: Limits kernel size in case
     * of large sigma, should be set to image width or height. For small values
     * of maxRadius, the kernel returned may have a larger radius, however.
     * @return A 2*n array. Array[0][n] is the kernel, decaying towards zero,
     * which would be reached at kernel.length (unless kernel size is limited by
     * maxRadius). Array[1][n] holds the sum over all kernel values > n,
     * including non-calculated values in case the kernel size is limited by
     * <code>maxRadius</code>.
     */
    public static float[][] makeGaussianKernel(final double sigma, final double accuracy, int maxRadius) {
